...
<body>
      ...
      <div class="app-container">
        ...
        <div class="app-hero-header">
          ...
          <div>
            ...
            <h3 class="fw-light">
              <span>Home</span> / <span class="menu-text">Deduction List</span>
            </h3>
          </div>
          <!-- Page Title end -->
                    ...
                    <thead>
                      ...
                      <?php
                            ...
                            while ($row = $query->fetch_assoc()) {
                                ...
                                <tr>
                                    <td>" . $counter++ . "</td> 
                                    <td>" . ucwords($row['deduction']) . "</td>
                                    <td>" . $row['deduction_type'] . "</td>
                                    <td>" . number_format($row['amount'], 2) . "</td>
                                     <td>" . $row['description'] . "</td>
                                    <td>
                                        <button class='btn btn-success btn-sm edit btn-flat' data-id='" . $row['dedID'] . "'>
                                            <i class='bi bi-pencil'></i>
                                        </button>
                                        <button class='btn btn-danger btn-sm delete btn-flat' data-id='" . $row['dedID'] . "'>
  ...
  <?php include 'modals/deduction_modal.php'; ?>
    ...
    function getRow(id) {
      $.ajax({
        ...
        success: function (response) {
          ...
          $('#decid').val(response.dedID);
          $('#edit_Deduction').val(response.deduction);
          $('#frequency').val(response.deduction_type);
          $('#amount').val(response.amount);
          $('#edit_Description').val(response.description);
          $('#del_al').val(response.dedID);
          $('#del_allow').html(response.deduction);
