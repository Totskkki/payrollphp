...
$sql = "SELECT *, SUM(amount) as total_amount FROM deductions where employee_id = $employee_id";
...
$sql = "SELECT *, SUM(num_hr) AS total_hr, attendance.employee_no AS empid, users.userid AS employee FROM attendance 
    LEFT JOIN employee ON employee.employee_id=attendance.employee_no 
    LEFT JOIN position ON position.employee_id= employee.employee_id 
    WHERE attendance.employee_no='$employee_id' AND date BETWEEN '$from' AND '$to' 
    GROUP BY attendance.employee_no 
...
while($row = $query->fetch_assoc()){
    ...
    $casql = "SELECT *, SUM(advance_amount) AS cashamount FROM cashadvance 
    WHERE employee_id='$empid' AND advance_date BETWEEN '$from' AND '$to'";
